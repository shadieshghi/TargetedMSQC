...
library(devtools)
devtools::install_github("/shadieshghi/TargetedMSQC")
```
...
- Few transitions: the peptide has fewer than 3 transitions. Some of the QC metrics in the `TargetedMSQC` require at least 3 transitions, so peptides with 2 transitions or fewer are removed from downstream QC analysis.
...
Using the `ViolinPlotQCSummary` on the data.merged output of this function, the violin plots of peak QC metric distributions can be overlaid with the labels associated with each peak using the `labels` parameter:

```{r, fig.cap="Violin plot summary visualizing the distribution of QC metrics of interest overlaid with QC status labels"}
violin.plots = ViolinPlotQCSummary(data.set.CSF$data.merged,
                                   runs = "all",
                                   features = feature.set,
                                   labels = "Status",
                                   font.size = 15)
...
model.rrf.CSF <- TrainQCModel(data.merged = data.training, 
